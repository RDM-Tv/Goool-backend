...
/* ---- Extrae goles con jugador y minuto del campo "details" de ESPN.
   Formato real: comp.details = [{ type:{text:'Goal'}, clock:{displayValue:"23'"},
   team:{id}, athletesInvolved:[{displayName}] }] ---- */
function extractGoalEvents(comp, homeId, awayId) {
  if (!Array.isArray(comp?.details)) return { home: [], away: [] };

  const goals = { home: [], away: [] };
  comp.details.forEach(ev => {
    const isGoal = /goal/i.test(ev.type?.text || '') && !/own goal/i.test(ev.type?.text || '');
    if (!isGoal) return;

    const scorer = ev.athletesInvolved?.[0]?.displayName || ev.athletesInvolved?.[0]?.shortName || 'Gol';
    const minute = ev.clock?.displayValue || '';
    const isPenalty = /penalty/i.test(ev.type?.text || '');
    const goalEntry = { scorer, minute, penalty: isPenalty };

    const teamId = ev.team?.id;
    if (teamId === homeId) goals.home.push(goalEntry);
    else if (teamId === awayId) goals.away.push(goalEntry);
  });
  return goals;
}

/* ---- Nombre legible del periodo de juego (1T, 2T, Entretiempo, etc.) ---- */
function periodLabel(status) {
  const period = status?.period;
  const stateName = (status?.type?.name || '').toUpperCase();
  if (stateName.includes('HALFTIME')) return 'Entretiempo';
  if (period === 1) return '1er Tiempo';
  if (period === 2) return '2do Tiempo';
  if (period === 3 || period === 4) return 'Tiempo Extra';
...
function parseMatches(data) {
  ...
  return data.events.map(ev => {
    ...
    const goalEvents = extractGoalEvents(comp, home?.id, away?.id);
    ...
    return {
      ...
      periodLabel: periodLabel(ev.status),
...
/* ---- Calcula estadísticas completas por equipo a partir de TODOS los
   partidos jugados (grupos + eliminatorias). Incluye goleadores reales.
   NOTA: la fuente de datos no provee tarjetas amarillas/rojas, así que
   esos campos no se inventan — se omiten honestamente. ---- */
function calculateTeamStats(rawMatches) {
  if (!Array.isArray(rawMatches)) return [];

  const teams = {};

  rawMatches.forEach(m => {
    if (!Array.isArray(m.score?.ft)) return;

    const home = (m.team1 || '').replace(/\s*\([A-Z]{3}\)$/, '');
    const away = (m.team2 || '').replace(/\s*\([A-Z]{3}\)$/, '');
    const [hg, ag] = m.score.ft;

    if (!teams[home]) teams[home] = { pj:0, g:0, em:0, p:0, pts:0, gf:0, gc:0, scorers:{} };
    if (!teams[away]) teams[away] = { pj:0, g:0, em:0, p:0, pts:0, gf:0, gc:0, scorers:{} };
    ...
    else              { h.em++; a.em++; h.pts += 1; a.pts += 1; }

    (m.goals1 || []).forEach(goal => {
      h.scorers[goal.name] = (h.scorers[goal.name] || 0) + 1;
    });
    (m.goals2 || []).forEach(goal => {
      a.scorers[goal.name] = (a.scorers[goal.name] || 0) + 1;
    });
  });
  ...
  return Object.entries(teams).map(([name, stats]) => {
    const topScorers = Object.entries(stats.scorers)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([scorerName, goals]) => ({ name: scorerName, goals }));
    ...
    return {
      ...
      pj: stats.pj, g: stats.g, em: stats.em, p: stats.p,
      pts: stats.pts,
      gf: stats.gf, gc: stats.gc, gd: stats.gf - stats.gc,
      winPct: stats.pj > 0 ? Math.round((stats.g / stats.pj) * 100) : 0,
      topScorers,
    };
  }).sort((a, b) => b.pts - a.pts || b.gd - a.gd || b.gf - a.gf);
}

/* ---- Tabla de goleadores globales del torneo ---- */
function calculateTopScorers(rawMatches) {
  if (!Array.isArray(rawMatches)) return [];
  ...
  rawMatches.forEach(m => {
    ...
    (m.goals1 || []).forEach(g => {
      if (!scorers[g.name]) scorers[g.name] = { goals: 0, team: home };
      scorers[g.name].goals++;
    });
    (m.goals2 || []).forEach(g => {
      if (!scorers[g.name]) scorers[g.name] = { goals: 0, team: away };
      scorers[g.name].goals++;
    });
  });
  ...
  return Object.entries(scorers)
    .map(([name, data], idx) => ({
      ...
      goals: data.goals,
      team:  data.team,
      flag:  countryFlag(data.team),
    }))
    .sort((a, b) => b.goals - a.goals)
    .map((s, i) => ({ ...s, pos: i + 1 }));
}

/* ---- Ranking general: todas las selecciones por puntos totales ---- */
function calculateOverallRanking(rawMatches) {
  return calculateTeamStats(rawMatches).map((t, i) => ({ ...t, pos: i + 1 }));
}
...
module.exports = { parseMatches, parseStandings, parseOpenFootballMatches, calculateStandingsFromMatches, calculateTeamStats, calculateTopScorers, calculateOverallRanking, parseNews, countryFlag };
