...
function calculateTeamStats(rawMatches) {
  ...
  const teams = {}; // { 'Ecuador': { pj, g, em, p, gf, gc, scorers: {} } }

  rawMatches.forEach(m => {
    if (!Array.isArray(m.score?.ft)) return; // solo partidos ya jugados

    const home = (m.team1 || '').replace(/\s*\([A-Z]{3}\)$/, '');
    const away = (m.team2 || '').replace(/\s*\([A-Z]{3}\)$/, '');
    const [hg, ag] = m.score.ft;

    if (!teams[home]) teams[home] = { pj:0, g:0, em:0, p:0, gf:0, gc:0, scorers:{} };
    if (!teams[away]) teams[away] = { pj:0, g:0, em:0, p:0, gf:0, gc:0, scorers:{} };
...
module.exports = { parseMatches, parseStandings, parseOpenFootballMatches, calculateStandingsFromMatches, calculateTeamStats, parseNews, countryFlag };
